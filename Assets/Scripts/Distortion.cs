...
public class Distortion : MonoBehaviour
{
    ...
    void Update()
    {
     //   if (Input.GetKeyDown(KeyCode.B))
      //  {
       //     for (int  i = 0; i < mobs.lvlsystem[mobs.Q].mobsObjects.Length;i++)
        //    {
                

         //   }
          //  Debug.Log(mobs.lvlsystem[v].mobsObjects.Length); 
          //  v++;
       // }
        ...
        for (int i = 0;i < mobs.lvlsystem[mobs.Q].mobsObjects.Length; i++)
        {
            DF[i]= Vector3.Distance(player.position, mobs.lvlsystem[mobs.Q].mobsObjects[i].transform.position);
